...
#define AIRWINDOWS_NAME "Suzan"
#define AIRWINDOWS_DESCRIPTION "A new type of ladder filter."
#define AIRWINDOWS_GUID NT_MULTICHAR( 'A','S','u','n' )
...
struct _kernel {
	...
	_airwindowsAlgorithm* owner;
		...
		float lowA, lowB, lowC;
		float bandA, bandB, bandC; //these are series poles
		float freqA, freqB;
		float resoA, resoB;
		float outA, outB; //these are control smoothing
		uint32_t fpd;
...
#define inNumChannels (1)
{
	...
	freqB = powf(GetParameter( kParam_A ),overallscale+1.0f)*1.22f;
	...
	while (nSampleFrames-- > 0) {
		...
		inputSampleL = (lowA-sin(bandC*0.5f)); //alternate airwindowsizationA
		
		lowB += freq*bandB; bandB += freq*((reso*inputSampleL)-lowB-(reso*bandB));
		inputSampleL = (lowB-sin(bandA*0.5f)); //alternate airwindowsizationB
