...
#define AIRWINDOWS_NAME "kRockstar"
#define AIRWINDOWS_DESCRIPTION "A realistic hall reverb that blends very well."
#define AIRWINDOWS_GUID NT_MULTICHAR( 'A','k','R','r' )
...
{ .name = "Positin", .min = 0, .max = 1000, .def = 1000, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
{ .name = "DownRez", .min = 0, .max = 1000, .def = 1000, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	overallscale /= 48000.0f; //special case for the reverbs: optimizes for downRez
	overallscale *= GetSampleRate();
	int slewsing = floor(overallscale*2.0f);
	if (slewsing < 2) slewsing = 2; if (slewsing > 32) slewsing = 32;	
	float reg6n = (1.0f-powf(1.0f-GetParameter( kParam_A ),2.0f))*0.0013425f;
	float regenMax = 0.0013425f;
	int start = (int)(GetParameter( kParam_B ) * 27.0f);
	
	float downRez = ((GetParameter( kParam_C )*(overallscale/(overallscale+0.99999f)))/overallscale);
	downRez = fmin(fmax(downRez,0.0005f),1.0f/overallscale);
	
	int bezFraction = (int)(1.0f/downRez);
	float bezTrim = (float)bezFraction/(bezFraction+1.0f);
	downRez = 0.99999999f / bezFraction;
	bezTrim = 1.0f-(downRez*bezTrim);
	...
	while (nSampleFrames-- > 0) {
		...
		dram->bez[bez_cycle] += downRez;
		dram->bez[bez_SampL] += (inputSampleL * downRez);
		dram->bez[bez_SampR] += (inputSampleR * downRez);
		if (dram->bez[bez_cycle] > bezTrim) { //hit the end point and we do a reverb sample
			dram->bez[bez_cycle] = 0.0f;
			
			float earlyFloor = fabs(inputSampleL) + fabs(inputSampleR);
			switch(earlyZero){
				case 0:
					earlyFloor = earlyFloor + fabs(dram->a3AL[c3AL]) + fabs(dram->a3AR[c3AR]);
					if (ld3A != early[start+3] && earlyFloor < 0.06125f)
					{
						for(int x = 0; x < d3A+2; x++) {dram->a3AL[x] = 0.0f; dram->a3AR[x] = 0.0f;}
						ld3A = early[start+3];
					} break;
				case 1:
					earlyFloor = earlyFloor + fabs(dram->a3BL[c3BL]) + fabs(dram->a3BR[c3BR]);
					if (ld3B != early[start+7] && earlyFloor < 0.06125f)
					{
						for(int x = 0; x < d3B+2; x++) {dram->a3BL[x] = 0.0f; dram->a3BR[x] = 0.0f;}
						ld3B = early[start+7];
					} break;
				case 2:
					earlyFloor = earlyFloor + fabs(dram->a3CL[c3CL]) + fabs(dram->a3CR[c3CR]);
					if (ld3C != early[start+8] && earlyFloor < 0.06125f)
					{
						for(int x = 0; x < d3C+2; x++) {dram->a3CL[x] = 0.0f; dram->a3CR[x] = 0.0f;}
						ld3C = early[start+8];
					} break;
				case 3:
					earlyFloor = earlyFloor + fabs(dram->a3DL[c3DL]) + fabs(dram->a3DR[c3DR]);
					if (ld3D != early[start+2] && earlyFloor < 0.06125f)
					{
						for(int x = 0; x < d3D+2; x++) {dram->a3DL[x] = 0.0f; dram->a3DR[x] = 0.0f;}
						ld3D = early[start+2];
					} break;
				case 4:
					earlyFloor = earlyFloor + fabs(dram->a3EL[c3EL]) + fabs(dram->a3ER[c3ER]);
					if (ld3E != early[start+4] && earlyFloor < 0.06125f)
					{
						for(int x = 0; x < d3E+2; x++) {dram->a3EL[x] = 0.0f; dram->a3ER[x] = 0.0f;}
						ld3E = early[start+4];
					} break;
				case 5:
					earlyFloor = earlyFloor + fabs(dram->a3FL[c3FL]) + fabs(dram->a3FR[c3FR]);
					if (ld3F != early[start+6] && earlyFloor < 0.06125f)
					{
						for(int x = 0; x < d3F+2; x++) {dram->a3FL[x] = 0.0f; dram->a3FR[x] = 0.0f;}
						ld3F = early[start+6];
					} break;
				case 6:
					earlyFloor = earlyFloor + fabs(dram->a3GL[c3GL]) + fabs(dram->a3GR[c3GR]);
					if (ld3G != early[start] && earlyFloor < 0.06125f)
					{
						for(int x = 0; x < d3G+2; x++) {dram->a3GL[x] = 0.0f; dram->a3GR[x] = 0.0f;}
						ld3G = early[start];
					} break;
				case 7:
					earlyFloor = earlyFloor + fabs(dram->a3HL[c3HL]) + fabs(dram->a3HR[c3HR]);
					if (ld3H != early[start+1] && earlyFloor < 0.06125f)
					{
						for(int x = 0; x < d3H+2; x++) {dram->a3HL[x] = 0.0f; dram->a3HR[x] = 0.0f;}
						ld3H = early[start+1];
					} break;
				case 8:
					earlyFloor = earlyFloor + fabs(dram->a3IL[c3IL]) + fabs(dram->a3IR[c3IR]);
					if (ld3I != early[start+5] && earlyFloor < 0.06125f)
					{
						for(int x = 0; x < d3I+2; x++) {dram->a3IL[x] = 0.0f; dram->a3IR[x] = 0.0f;}
						ld3I = early[start+5];
					} break;
			}
			...
			float hA = dram->a3AL[c3AL-((c3AL > ld3A)?c3AL+1:0)];
			float hB = dram->a3BL[c3BL-((c3BL > ld3B)?c3BL+1:0)];
			float hC = dram->a3CL[c3CL-((c3CL > ld3C)?c3CL+1:0)];
			float hD = dram->a3CR[c3CR-((c3CR > ld3C)?c3CR+1:0)];
			float hE = dram->a3FR[c3FR-((c3FR > ld3F)?c3FR+1:0)];
			float hF = dram->a3IR[c3IR-((c3IR > ld3I)?c3IR+1:0)];
			
			dram->a3DL[c3DL] = (((hB + hC) * -2.0f) + hA);
			dram->a3EL[c3EL] = (((hA + hC) * -2.0f) + hB);
			dram->a3FL[c3FL] = (((hA + hB) * -2.0f) + hC);
			dram->a3BR[c3BR] = (((hE + hF) * -2.0f) + hD);
			dram->a3ER[c3ER] = (((hD + hF) * -2.0f) + hE);
			dram->a3HR[c3HR] = (((hD + hE) * -2.0f) + hF);
			...
			hA = dram->a3DL[c3DL-((c3DL > ld3D)?c3DL+1:0)];
			hB = dram->a3EL[c3EL-((c3EL > ld3E)?c3EL+1:0)];
			hC = dram->a3FL[c3FL-((c3FL > ld3F)?c3FL+1:0)];
			hD = dram->a3BR[c3BR-((c3BR > ld3B)?c3BR+1:0)];
			hE = dram->a3ER[c3ER-((c3ER > ld3E)?c3ER+1:0)];
			hF = dram->a3HR[c3HR-((c3HR > ld3H)?c3HR+1:0)];
			
			dram->a3GL[c3GL] = (((hB + hC) * -2.0f) + hA);
			dram->a3HL[c3HL] = (((hA + hC) * -2.0f) + hB);
			dram->a3IL[c3IL] = (((hA + hB) * -2.0f) + hC);
			dram->a3AR[c3AR] = (((hE + hF) * -2.0f) + hD);
			dram->a3DR[c3DR] = (((hD + hF) * -2.0f) + hE);
			dram->a3GR[c3GR] = (((hD + hE) * -2.0f) + hF);
			...
			hA = dram->a3GL[c3GL-((c3GL > ld3G)?c3GL+1:0)];
			hB = dram->a3HL[c3HL-((c3HL > ld3H)?c3HL+1:0)];
			hC = dram->a3IL[c3IL-((c3IL > ld3I)?c3IL+1:0)];
			hD = dram->a3AR[c3AR-((c3AR > ld3A)?c3AR+1:0)];
			hE = dram->a3DR[c3DR-((c3DR > ld3D)?c3DR+1:0)];
			hF = dram->a3GR[c3GR-((c3GR > ld3G)?c3GR+1:0)];
			
			float earlyReflectionL = (((hB + hC) * -2.0f) + hA)*-0.0625f;
			float earlyReflectionR = (((hE + hF) * -2.0f) + hD)*-0.0625f;
			
			float applyNonlin = 1.0f-fmin(fmax(fabs(inputSampleL),fabs(inputSampleR)),1.0f);
			applyNonlin = 1.0f-(applyNonlin*applyNonlin);
			...
			if (roomTimerL > 13) {
				roomTimerL = 0;
				if (roomNoiseL > 0.0f) roomNoiseL -= fabs((fpdL / (float)UINT32_MAX)-0.5f)*0.125f;
				else roomNoiseL += fabs((fpdL / (float)UINT32_MAX)-0.5f)*0.125f;
			} else roomNoiseL += ((fpdL / (float)UINT32_MAX)-0.5f)*0.125f;
			//roomNoiseL governs the added 'room tone' noise. 0.125f gives you not an overwhelming
			...
			if (roomTimerR > 4) {
				roomTimerR = 0;
				if (roomNoiseR > fmin(reg6n+(applyNonlin*0.0002f),regenMax)) roomNoiseR -= fabs((fpdR / (float)UINT32_MAX)-0.5f)*0.000007629394531f;
				else roomNoiseR += fabs((fpdR / (float)UINT32_MAX)-0.5f)*0.000007629394531f;
			} else roomNoiseR += ((fpdR / (float)UINT32_MAX)-0.5f)*0.000007629394531f;
			...
			//and the scaling of roomNoiseR (as added to reg6n) is sqrt(wind MPH)*0.00002f
						
			dram->a6AL[c6AL] = inputSampleL + ((f6BL+roomNoiseL) * roomNoiseR);
			dram->a6BL[c6BL] = inputSampleL + (f6CL * roomNoiseR);
			dram->a6CL[c6CL] = inputSampleL + (f6DL * roomNoiseR);
			dram->a6DL[c6DL] = inputSampleL + (f6EL * roomNoiseR);
			dram->a6EL[c6EL] = inputSampleL + (f6FL * roomNoiseR);
			dram->a6FL[c6FL] = inputSampleL + (f6AL * roomNoiseR);
			
			dram->a6FR[c6FR] = inputSampleR + ((f6LR+roomNoiseL) * roomNoiseR);
			dram->a6LR[c6LR] = inputSampleR + (f6RR * roomNoiseR);
			dram->a6RR[c6RR] = inputSampleR + (f6XR * roomNoiseR);
			dram->a6XR[c6XR] = inputSampleR + (f6ZER * roomNoiseR);
			dram->a6ZER[c6ZER] = inputSampleR + (f6ZKR * roomNoiseR);
			dram->a6ZKR[c6ZKR] = inputSampleR + (f6FR * roomNoiseR);
			...
			hA = dram->a6AL[c6AL-((c6AL > d6A)?d6A+1:0)];
			hB = dram->a6BL[c6BL-((c6BL > d6B)?d6B+1:0)];
			hC = dram->a6CL[c6CL-((c6CL > d6C)?d6C+1:0)];
			hD = dram->a6DL[c6DL-((c6DL > d6D)?d6D+1:0)];
			hE = dram->a6EL[c6EL-((c6EL > d6E)?d6E+1:0)];
			hF = dram->a6FL[c6FL-((c6FL > d6F)?d6F+1:0)];
			
			dram->a6GL[c6GL] = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			dram->a6HL[c6HL] = ((hB*2.0f) - (hA + hC + hD + hE + hF));
			dram->a6IL[c6IL] = ((hC*2.0f) - (hA + hB + hD + hE + hF));
			dram->a6JL[c6JL] = ((hD*2.0f) - (hA + hB + hC + hE + hF));
			dram->a6KL[c6KL] = ((hE*2.0f) - (hA + hB + hC + hD + hF));
			dram->a6LL[c6LL] = ((hF*2.0f) - (hA + hB + hC + hD + hE));
			...
			hA = dram->a6GL[c6GL-((c6GL > d6G)?d6G+1:0)];
			hB = dram->a6HL[c6HL-((c6HL > d6H)?d6H+1:0)];
			hC = dram->a6IL[c6IL-((c6IL > d6I)?d6I+1:0)];
			hD = dram->a6JL[c6JL-((c6JL > d6J)?d6J+1:0)];
			hE = dram->a6KL[c6KL-((c6KL > d6K)?d6K+1:0)];
			hF = dram->a6LL[c6LL-((c6LL > d6L)?d6L+1:0)];
			
			dram->a6ML[c6ML] = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			dram->a6NL[c6NL] = ((hB*2.0f) - (hA + hC + hD + hE + hF));
			dram->a6OL[c6OL] = ((hC*2.0f) - (hA + hB + hD + hE + hF));
			dram->a6PL[c6PL] = ((hD*2.0f) - (hA + hB + hC + hE + hF));
			dram->a6QL[c6QL] = ((hE*2.0f) - (hA + hB + hC + hD + hF));
			dram->a6RL[c6RL] = ((hF*2.0f) - (hA + hB + hC + hD + hE));
			...
			hA = dram->a6ML[c6ML-((c6ML > d6M)?d6M+1:0)];
			hB = dram->a6NL[c6NL-((c6NL > d6N)?d6N+1:0)];
			hC = dram->a6OL[c6OL-((c6OL > d6O)?d6O+1:0)];
			hD = dram->a6PL[c6PL-((c6PL > d6P)?d6P+1:0)];
			hE = dram->a6QL[c6QL-((c6QL > d6Q)?d6Q+1:0)];
			hF = dram->a6RL[c6RL-((c6RL > d6R)?d6R+1:0)];
			
			dram->a6SL[c6SL] = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			dram->a6TL[c6TL] = ((hB*2.0f) - (hA + hC + hD + hE + hF));
			dram->a6UL[c6UL] = ((hC*2.0f) - (hA + hB + hD + hE + hF));
			dram->a6VL[c6VL] = ((hD*2.0f) - (hA + hB + hC + hE + hF));
			dram->a6WL[c6WL] = ((hE*2.0f) - (hA + hB + hC + hD + hF));
			dram->a6XL[c6XL] = ((hF*2.0f) - (hA + hB + hC + hD + hE));
			...
			hA = dram->a6SL[c6SL-((c6SL > d6S)?d6S+1:0)];
			hB = dram->a6TL[c6TL-((c6TL > d6T)?d6T+1:0)];
			hC = dram->a6UL[c6UL-((c6UL > d6U)?d6U+1:0)];
			hD = dram->a6VL[c6VL-((c6VL > d6V)?d6V+1:0)];
			hE = dram->a6WL[c6WL-((c6WL > d6W)?d6W+1:0)];
			hF = dram->a6XL[c6XL-((c6XL > d6X)?d6X+1:0)];
			
			dram->a6YL[c6YL] = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			dram->a6ZAL[c6ZAL] = ((hB*2.0f) - (hA + hC + hD + hE + hF));
			dram->a6ZBL[c6ZBL] = ((hC*2.0f) - (hA + hB + hD + hE + hF));
			dram->a6ZCL[c6ZCL] = ((hD*2.0f) - (hA + hB + hC + hE + hF));
			dram->a6ZDL[c6ZDL] = ((hE*2.0f) - (hA + hB + hC + hD + hF));
			dram->a6ZEL[c6ZEL] = ((hF*2.0f) - (hA + hB + hC + hD + hE));
			...
			hA = dram->a6YL[c6YL-((c6YL > d6Y)?d6Y+1:0)];
			hB = dram->a6ZAL[c6ZAL-((c6ZAL > d6ZA)?d6ZA+1:0)];
			hC = dram->a6ZBL[c6ZBL-((c6ZBL > d6ZB)?d6ZB+1:0)];
			hD = dram->a6ZCL[c6ZCL-((c6ZCL > d6ZC)?d6ZC+1:0)];
			hE = dram->a6ZDL[c6ZDL-((c6ZDL > d6ZD)?d6ZD+1:0)];
			hF = dram->a6ZEL[c6ZEL-((c6ZEL > d6ZE)?d6ZE+1:0)];
			
			dram->a6ZFL[c6ZFL] = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			dram->a6ZGL[c6ZGL] = ((hB*2.0f) - (hA + hC + hD + hE + hF));
			dram->a6ZHL[c6ZHL] = ((hC*2.0f) - (hA + hB + hD + hE + hF));
			dram->a6ZIL[c6ZIL] = ((hD*2.0f) - (hA + hB + hC + hE + hF));
			dram->a6ZJL[c6ZJL] = ((hE*2.0f) - (hA + hB + hC + hD + hF));
			dram->a6ZKL[c6ZKL] = ((hF*2.0f) - (hA + hB + hC + hD + hE));
			...
			hA = dram->a6ZFL[c6ZFL-((c6ZFL > d6ZF)?d6ZF+1:0)];
			hB = dram->a6ZGL[c6ZGL-((c6ZGL > d6ZG)?d6ZG+1:0)];
			hC = dram->a6ZHL[c6ZHL-((c6ZHL > d6ZH)?d6ZH+1:0)];
			hD = dram->a6ZIL[c6ZIL-((c6ZIL > d6ZI)?d6ZI+1:0)];
			hE = dram->a6ZJL[c6ZJL-((c6ZJL > d6ZJ)?d6ZJ+1:0)];
			hF = dram->a6ZKL[c6ZKL-((c6ZKL > d6ZK)?d6ZK+1:0)];
			
			f6FR = (((hA*2.0f) - (hB + hC + hD + hE + hF))+avg6R)*0.5f;
			avg6R = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			...
			inputSampleL = ((hA*2.0f) - (hB + hC + hD + hE + hF))*0.001953125f;
			...
			hA = dram->a6FR[c6FR-((c6FR > d6F)?d6F+1:0)];
			hB = dram->a6LR[c6LR-((c6LR > d6L)?d6L+1:0)];
			hC = dram->a6RR[c6RR-((c6RR > d6R)?d6R+1:0)];
			hD = dram->a6XR[c6XR-((c6XR > d6X)?d6X+1:0)];
			hE = dram->a6ZER[c6ZER-((c6ZER > d6ZE)?d6ZE+1:0)];
			hF = dram->a6ZKR[c6ZKR-((c6ZKR > d6ZK)?d6ZK+1:0)];
			
			dram->a6ER[c6ER] = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			dram->a6KR[c6KR] = ((hB*2.0f) - (hA + hC + hD + hE + hF));
			dram->a6QR[c6QR] = ((hC*2.0f) - (hA + hB + hD + hE + hF));
			dram->a6WR[c6WR] = ((hD*2.0f) - (hA + hB + hC + hE + hF));
			dram->a6ZDR[c6ZDR] = ((hE*2.0f) - (hA + hB + hC + hD + hF));			
			dram->a6ZJR[c6ZJR] = ((hF*2.0f) - (hA + hB + hC + hD + hE));			
			...
			hA = dram->a6ER[c6ER-((c6ER > d6E)?d6E+1:0)];
			hB = dram->a6KR[c6KR-((c6KR > d6K)?d6K+1:0)];
			hC = dram->a6QR[c6QR-((c6QR > d6Q)?d6Q+1:0)];
			hD = dram->a6WR[c6WR-((c6WR > d6W)?d6W+1:0)];
			hE = dram->a6ZDR[c6ZDR-((c6ZDR > d6ZD)?d6ZD+1:0)];
			hF = dram->a6ZJR[c6ZJR-((c6ZJR > d6ZJ)?d6ZJ+1:0)];
			
			dram->a6DR[c6DR] = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			dram->a6JR[c6JR] = ((hB*2.0f) - (hA + hC + hD + hE + hF));
			dram->a6PR[c6PR] = ((hC*2.0f) - (hA + hB + hD + hE + hF));
			dram->a6VR[c6VR] = ((hD*2.0f) - (hA + hB + hC + hE + hF));
			dram->a6ZCR[c6ZCR] = ((hE*2.0f) - (hA + hB + hC + hD + hF));
			dram->a6ZIR[c6ZIR] = ((hF*2.0f) - (hA + hB + hC + hD + hE));
			...
			hA = dram->a6DR[c6DR-((c6DR > d6D)?d6D+1:0)];
			hB = dram->a6JR[c6JR-((c6JR > d6J)?d6J+1:0)];
			hC = dram->a6PR[c6PR-((c6PR > d6P)?d6P+1:0)];
			hD = dram->a6VR[c6VR-((c6VR > d6V)?d6V+1:0)];
			hE = dram->a6ZCR[c6ZCR-((c6ZCR > d6ZC)?d6ZC+1:0)];
			hF = dram->a6ZIR[c6ZIR-((c6ZIR > d6ZI)?d6ZI+1:0)];
			
			dram->a6CR[c6CR] = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			dram->a6IR[c6IR] = ((hB*2.0f) - (hA + hC + hD + hE + hF));
			dram->a6OR[c6OR] = ((hC*2.0f) - (hA + hB + hD + hE + hF));
			dram->a6UR[c6UR] = ((hD*2.0f) - (hA + hB + hC + hE + hF));
			dram->a6ZBR[c6ZBR] = ((hE*2.0f) - (hA + hB + hC + hD + hF));
			dram->a6ZHR[c6ZHR] = ((hF*2.0f) - (hA + hB + hC + hD + hE));
			...
			hA = dram->a6CR[c6CR-((c6CR > d6C)?d6C+1:0)];
			hB = dram->a6IR[c6IR-((c6IR > d6I)?d6I+1:0)];
			hC = dram->a6OR[c6OR-((c6OR > d6O)?d6O+1:0)];
			hD = dram->a6UR[c6UR-((c6UR > d6U)?d6U+1:0)];
			hE = dram->a6ZBR[c6ZBR-((c6ZBR > d6ZB)?d6ZB+1:0)];
			hF = dram->a6ZHR[c6ZHR-((c6ZHR > d6ZH)?d6ZH+1:0)];
			
			dram->a6BR[c6BR] = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			dram->a6HR[c6HR] = ((hB*2.0f) - (hA + hC + hD + hE + hF));
			dram->a6NR[c6NR] = ((hC*2.0f) - (hA + hB + hD + hE + hF));
			dram->a6TR[c6TR] = ((hD*2.0f) - (hA + hB + hC + hE + hF));
			dram->a6ZAR[c6ZAR] = ((hE*2.0f) - (hA + hB + hC + hD + hF));
			dram->a6ZGR[c6ZGR] = ((hF*2.0f) - (hA + hB + hC + hD + hE));
			...
			hA = dram->a6BR[c6BR-((c6BR > d6B)?d6B+1:0)];
			hB = dram->a6HR[c6HR-((c6HR > d6H)?d6H+1:0)];
			hC = dram->a6NR[c6NR-((c6NR > d6N)?d6N+1:0)];
			hD = dram->a6TR[c6TR-((c6TR > d6T)?d6T+1:0)];
			hE = dram->a6ZAR[c6ZAR-((c6ZAR > d6ZA)?d6ZA+1:0)];
			hF = dram->a6ZGR[c6ZGR-((c6ZGR > d6ZG)?d6ZG+1:0)];
			
			dram->a6AR[c6AR] = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			dram->a6GR[c6GR] = ((hB*2.0f) - (hA + hC + hD + hE + hF));
			dram->a6MR[c6MR] = ((hC*2.0f) - (hA + hB + hD + hE + hF));
			dram->a6SR[c6SR] = ((hD*2.0f) - (hA + hB + hC + hE + hF));
			dram->a6YR[c6YR] = ((hE*2.0f) - (hA + hB + hC + hD + hF));
			dram->a6ZFR[c6ZFR] = ((hF*2.0f) - (hA + hB + hC + hD + hE));
			...
			hA = dram->a6AR[c6AR-((c6AR > d6A)?d6A+1:0)];
			hB = dram->a6GR[c6GR-((c6GR > d6G)?d6G+1:0)];
			hC = dram->a6MR[c6MR-((c6MR > d6M)?d6M+1:0)];
			hD = dram->a6SR[c6SR-((c6SR > d6S)?d6S+1:0)];
			hE = dram->a6YR[c6YR-((c6YR > d6Y)?d6Y+1:0)];
			hF = dram->a6ZFR[c6ZFR-((c6ZFR > d6ZF)?d6ZF+1:0)];
			
			f6AL = (((hA*2.0f) - (hB + hC + hD + hE + hF))+avg6L)*0.5f;
			avg6L = ((hA*2.0f) - (hB + hC + hD + hE + hF));
			...
			inputSampleR = ((hA*2.0f) - (hB + hC + hD + hE + hF))*0.001953125f;
		...
		float X = dram->bez[bez_cycle];
		inputSampleL = (dram->bez[bez_BL]+(dram->bez[bez_CL]*(1.0f-X)*(1.0f-X))+(dram->bez[bez_BL]*2.0f*(1.0f-X)*X)+(dram->bez[bez_AL]*X*X))*-0.0625f;
		inputSampleR = (dram->bez[bez_BR]+(dram->bez[bez_CR]*(1.0f-X)*(1.0f-X))+(dram->bez[bez_BR]*2.0f*(1.0f-X)*X)+(dram->bez[bez_AR]*X*X))*-0.0625f;
		...
		float avgSlewL = fmin(fabs(lastDarkL-inputSampleL)*0.096f*overallscale,1.0f);
		...
		float avgSlewR = fmin(fabs(lastDarkR-inputSampleR)*0.096f*overallscale,1.0f);
