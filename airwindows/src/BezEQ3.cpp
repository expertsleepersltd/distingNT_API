...
#define AIRWINDOWS_NAME "BezEQ3"
#define AIRWINDOWS_DESCRIPTION "Refines the concept into four vivid, unified bands."
#define AIRWINDOWS_GUID NT_MULTICHAR( 'A','B','e','3' )
...
#define inNumChannels (1)
{
	...
	float highGain = (powf(GetParameter( kParam_A )+0.5f,3.0f)+3.0f)*0.25f;
	float hmidGain = (powf(GetParameter( kParam_B )+0.5f,3.0f)+3.0f)*0.25f;
	float lmidGain = (powf(GetParameter( kParam_C )+0.5f,3.0f)+3.0f)*0.25f;
	float bassGain = (powf(GetParameter( kParam_D )+0.5f,3.0f)+3.0f)*0.25f;
	float trebleRef = (1.0f-powf(1.0f-GetParameter( kParam_E ),2.0f))-0.5f;
	float highmidRef = GetParameter( kParam_F )-0.5f;
	float lowmidRef = GetParameter( kParam_G )-0.5f;
	float bassRef = powf(GetParameter( kParam_H ),2.0f)-0.5f;
	float HMderez = powf(0.75f + ((trebleRef+trebleRef+trebleRef+highmidRef)*0.125f),1.618f) /overallscale;
	float LMderez = powf(0.25f + ((lowmidRef+bassRef+bassRef+bassRef)*0.125f),1.618f) /overallscale;
	float Mderez = powf((HMderez*0.5f)+(LMderez*0.5f)+((highmidRef+lowmidRef)*0.125f),1.618f) /overallscale;
	int stepped = 999999; if (HMderez > 0.000001f) stepped = (int)(1.0f/HMderez);
	HMderez = 1.0f / stepped;
	...
	while (nSampleFrames-- > 0) {
		...
		for (int p = 0; p < 3; p++) {
			...
			float X = bezEQ[bez_HMcycle][p]*HMtrim;
