...
#define AIRWINDOWS_GUID NT_MULTICHAR( 'A','k','G','2' )
...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	float freq = GetParameter( kParam_C )+0.02f;
	...
	while (nSampleFrames-- > 0) {
		...
		if (dram->bez[bez_cycle] > 1.0f) { //hit the end point and we do a reverb sample
			...
			float o3AL = dram->a3AL[c3AL-((c3AL > ld3A)?c3AL+1:0)];
			float o3BL = dram->a3BL[c3BL-((c3BL > ld3B)?c3BL+1:0)];
			float o3CL = dram->a3CL[c3CL-((c3CL > ld3C)?c3CL+1:0)];
			float o3CR = dram->a3CR[c3CR-((c3CR > ld3C)?c3CR+1:0)];
			float o3FR = dram->a3FR[c3FR-((c3FR > ld3F)?c3FR+1:0)];
			float o3IR = dram->a3IR[c3IR-((c3IR > ld3I)?c3IR+1:0)];
			...
			float o3DL = dram->a3DL[c3DL-((c3DL > ld3D)?c3DL+1:0)];
			float o3EL = dram->a3EL[c3EL-((c3EL > ld3E)?c3EL+1:0)];
			float o3FL = dram->a3FL[c3FL-((c3FL > ld3F)?c3FL+1:0)];
			float o3BR = dram->a3BR[c3BR-((c3BR > ld3B)?c3BR+1:0)];
			float o3ER = dram->a3ER[c3ER-((c3ER > ld3E)?c3ER+1:0)];
			float o3HR = dram->a3HR[c3HR-((c3HR > ld3H)?c3HR+1:0)];
			...
			float o3GL = dram->a3GL[c3GL-((c3GL > ld3G)?c3GL+1:0)];
			float o3HL = dram->a3HL[c3HL-((c3HL > ld3H)?c3HL+1:0)];
			float o3IL = dram->a3IL[c3IL-((c3IL > ld3I)?c3IL+1:0)];
			float o3AR = dram->a3AR[c3AR-((c3AR > ld3A)?c3AR+1:0)];
			float o3DR = dram->a3DR[c3DR-((c3DR > ld3D)?c3DR+1:0)];
			float o3GR = dram->a3GR[c3GR-((c3GR > ld3G)?c3GR+1:0)];
			...
			if (freq < 1.0f) {
				float di = fabs(freq*(1.0f+(inputSampleL*0.125f))); if (di > 1.0f) di = 1.0f;
				float slew = ((inputSampleL - dram->pear[prevSampL1]) + dram->pear[prevSlewL1])*di*0.5f;
				dram->pear[prevSampL1] = inputSampleL = (di * inputSampleL) + ((1.0f-di) * (dram->pear[prevSampL1] + dram->pear[prevSlewL1]));
				dram->pear[prevSlewL1] = slew;
				di = fabs(freq*(1.0f+(inputSampleL))); if (di > 1.0f) di = 1.0f;
				slew = ((inputSampleL - dram->pear[prevSampL2]) + dram->pear[prevSlewL2])*di*0.5f;
				dram->pear[prevSampL2] = inputSampleL = (di * inputSampleL) + ((1.0f-di) * (dram->pear[prevSampL2] + dram->pear[prevSlewL2]));
				dram->pear[prevSlewL2] = slew;
				di = fabs(freq*(1.0f+(inputSampleL))); if (di > 1.0f) di = 1.0f;
				slew = ((inputSampleL - dram->pear[prevSampL3]) + dram->pear[prevSlewL3])*di*0.5f;
				dram->pear[prevSampL3] = inputSampleL = (di * inputSampleL) + ((1.0f-di) * (dram->pear[prevSampL3] + dram->pear[prevSlewL3]));
				dram->pear[prevSlewL3] = slew;
				
				di = fabs(freq*(1.0f+(inputSampleR*0.125f))); if (di > 1.0f) di = 1.0f;
				slew = ((inputSampleR - dram->pear[prevSampR1]) + dram->pear[prevSlewR1])*di*0.5f;
				dram->pear[prevSampR1] = inputSampleR = (di * inputSampleR) + ((1.0f-di) * (dram->pear[prevSampR1] + dram->pear[prevSlewR1]));
				dram->pear[prevSlewR1] = slew;
				di = fabs(freq*(1.0f+(inputSampleR))); if (di > 1.0f) di = 1.0f;
				slew = ((inputSampleR - dram->pear[prevSampR2]) + dram->pear[prevSlewR2])*di*0.5f;
				dram->pear[prevSampR2] = inputSampleR = (di * inputSampleR) + ((1.0f-di) * (dram->pear[prevSampR2] + dram->pear[prevSlewR2]));
				dram->pear[prevSlewR2] = slew;
				di = fabs(freq*(1.0f+(inputSampleR))); if (di > 1.0f) di = 1.0f;
				slew = ((inputSampleR - dram->pear[prevSampR3]) + dram->pear[prevSlewR3])*di*0.5f;
				dram->pear[prevSampR3] = inputSampleR = (di * inputSampleR) + ((1.0f-di) * (dram->pear[prevSampR3] + dram->pear[prevSlewR3]));
				dram->pear[prevSlewR3] = slew;
			}
			...
			f6AL = (f6AL+f6AL+f6AL+fabs(avg6L))*0.25f; avg6L = f6AL;
			f6FR = (f6FR+f6FR+f6FR+fabs(avg6R))*0.25f; avg6R = f6FR;
		...
		float CBAL = (dram->bez[bez_BL]+(CBL*(1.0f-X))+(BAL*X))*-0.0625f;
		float CBAR = (dram->bez[bez_BR]+(CBR*(1.0f-X))+(BAR*X))*-0.0625f;
