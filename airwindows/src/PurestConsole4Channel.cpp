...
#define AIRWINDOWS_NAME "PurestConsole4Channel"
#define AIRWINDOWS_DESCRIPTION "PurestConsole4Channel"
...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	while (nSampleFrames-- > 0) {
		...
		if (dram->bezier[bezier_cycle] > bezierTrim) {dram->bezier[bezier_cycle] = 0.0f;
			...
			float gain = GetParameter( kParam_FAD )*1.414213562373094f; //Pan will pad this
		...
		inputSampleR += temp*0.00833333333333333333333333333333333f; temp *= X;
		inputSampleR -= temp*0.00019841269841269841269841269841269f; temp *= X;
		inputSampleR += temp*0.00000275573192239858906525573192239f; temp *= X;
		inputSampleR -= temp*0.00000002505210838544171877521083854f; temp *= X;
		inputSampleR += temp*0.00000000016059043836821614599392377f; temp *= X;
		inputSampleR -= temp*0.00000000000076471637318198164759011f; temp *= X;
		inputSampleR += temp*0.00000000000000281145725434552076319f; temp *= X;
		inputSampleR -= temp*0.00000000000000000822063524662432971f; temp *= X;
		inputSampleR += temp*0.00000000000000000001957294106339126f;
