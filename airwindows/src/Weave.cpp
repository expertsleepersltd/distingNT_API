...
#define AIRWINDOWS_NAME "Weave"
#define AIRWINDOWS_DESCRIPTION "Weave"
#define AIRWINDOWS_GUID NT_MULTICHAR( 'A','W','e','e' )
...
#define inNumChannels (1)
{
	...
	while (nSampleFrames-- > 0) {
		...
		if (inputSample != 0.0f) {
			...
			inputSample *= 1.27f;
			float weave = sqrt(fabs(inputSample));
			float spiral = inputSample * weave;
			float X = spiral; X *= X;
			float temp = spiral * X;
