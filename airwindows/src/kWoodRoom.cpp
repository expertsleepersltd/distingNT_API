...
#define AIRWINDOWS_NAME "kWoodRoom"
#define AIRWINDOWS_DESCRIPTION "A small wooden performing space."
#define AIRWINDOWS_GUID NT_MULTICHAR( 'A','k','W','o' )
...
{ .name = "Filter", .min = 0, .max = 1000, .def = 250, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
{ .name = "EarlyRf", .min = 0, .max = 1000, .def = 500, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
{ .name = "Positin", .min = 0, .max = 1000, .def = 750, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	float reg6n = (1.0f-powf(1.0f-GetParameter( kParam_A ),1.618033988749894f))*fdb6ck;
	...
	if (derez > 1.0f) {  // has full rez at center, stepped
		stepped = false; // to left, continuous to right
		derez = 1.0f-(derez-1.0f);
	...
	if (stepped) { //this hard-locks derez to exact subdivisions of 1.0f
		...
		derez /= (2.0f/powf(overallscale,0.5f-((overallscale-1.0f)*0.0375f)));
		bezTrim = 1.0f-powf(derez*0.5f,1.0f/(derez*0.5f));
	} //the revision more accurately connects the bezier curves
	
	float derezFreq = GetParameter( kParam_C )*2.0f;
	bool steppedFreq = true; // Revised Bezier Undersampling
	if (derezFreq > 1.0f) {  // has full rez at center, stepped
		steppedFreq = false; // to left, continuous to right
		derezFreq = 1.0f-(derezFreq-1.0f);
	} //if it's set up like that it's the revised algorithm
	derezFreq = fmin(fmax(derezFreq,0.0005f),1.0f); //note: no overallscale, already inside undersampling
	...
	if (steppedFreq) { //this hard-locks derez to exact subdivisions of 1.0f
		...
		bezFreqTrim = 1.0f-powf(derezFreq*0.5f,1.0f/(derezFreq*0.5f));
	} //the revision more accurately connects the bezier curves
	
	float earlyLoudness = powf(GetParameter( kParam_D ),2.0f);
	int start = (int)(GetParameter( kParam_E ) * 27.0f);
	...
	while (nSampleFrames-- > 0) {
		...
		if (dram->bez[bez_cycle] > 1.0f) { //hit the end point and we do a reverb sample
			...
			dram->a3AL[c3AL] = inputSampleL;// + (f3AL * reg3n);
			dram->a3BL[c3BL] = inputSampleL;// + (f3BL * reg3n);
			dram->a3CL[c3CL] = inputSampleL;// + (f3CL * reg3n);
			
			dram->a3CR[c3CR] = inputSampleR;// + (f3CR * reg3n);
			dram->a3FR[c3FR] = inputSampleR;// + (f3FR * reg3n);
			dram->a3IR[c3IR] = inputSampleR;// + (f3IR * reg3n);
			...
			inputSampleL = (dram->bezF[bez_BL]+(CBLfreq*(1.0f-X))+(BALfreq*X))*0.125f;
			float CBRfreq = (dram->bezF[bez_CR]*(1.0f-X))+(dram->bezF[bez_BR]*X);
			float BARfreq = (dram->bezF[bez_BR]*(1.0f-X))+(dram->bezF[bez_AR]*X);
			inputSampleR = (dram->bezF[bez_BR]+(CBRfreq*(1.0f-X))+(BARfreq*X))*0.125f;
			
			inputSampleL = dram->bezF[bez_IIRL] = (inputSampleL*derezFreq)+(dram->bezF[bez_IIRL]*(1.0f-derezFreq));
			inputSampleR = dram->bezF[bez_IIRR] = (inputSampleR*derezFreq)+(dram->bezF[bez_IIRR]*(1.0f-derezFreq));
		...
		inputSampleL = dram->bez[bez_IIRL] = (inputSampleL*derez)+(dram->bez[bez_IIRL]*(1.0f-derez));
		inputSampleR = dram->bez[bez_IIRR] = (inputSampleR*derez)+(dram->bez[bez_IIRR]*(1.0f-derez));
