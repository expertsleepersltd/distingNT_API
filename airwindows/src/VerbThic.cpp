...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	while (nSampleFrames-- > 0) {
		...
		if (dram->bez[bez_cycle] > 1.0f) { //hit the end point and we do a reverb sample
			...
			float temp = inputSampleL * X;
			inputSampleL -= (temp*0.125f); temp *= X;
			inputSampleL += (temp*0.0078125f); temp *= X;
			inputSampleL -= (temp*0.000244140625f); temp *= X;
			inputSampleL += (temp*0.000003814697265625f); temp *= X;
			inputSampleL -= (temp*0.0000000298023223876953125f); temp *= X;
			...
			temp = inputSampleR * X;
			inputSampleR -= (temp*0.125f); temp *= X;
			inputSampleR += (temp*0.0078125f); temp *= X;
			inputSampleR -= (temp*0.000244140625f); temp *= X;
			inputSampleR += (temp*0.000003814697265625f); temp *= X;
			inputSampleR -= (temp*0.0000000298023223876953125f); temp *= X;
			//purestsaturation: sine, except all the corrections
			//retain mantissa of a float increasing power function
